...
public class PermalinkGeneratorService {
    ...
    private void updateVanityForNode(JCRNodeWrapper node, String language, JCRSiteNode site, JCRSessionWrapper session) throws RepositoryException {
        updateVanityForNode(node, language, site, session, false);
    }

    private void updateVanityForNode(JCRNodeWrapper node, String language, JCRSiteNode site, JCRSessionWrapper session, boolean forceRegen) throws RepositoryException {
        if (node.hasProperty("j:isHomePage") && node.getProperty("j:isHomePage").getBoolean()) {
            logger.debug("Skip {} [{}]: homepage", node.getPath(), language); return;
        }
        if (node.isNodeType(MIXIN_PERMALINK_EXCLUDED)) {
            logger.debug("Skip {} [{}]: permalinkExcluded", node.getPath(), language); return;
        }
        if (isExcludedPath(node, site)) {
            logger.debug("Skip {} [{}]: excluded path", node.getPath(), language); return;
        }
        ...
        if (!JCRContentUtils.isADisplayableNode(node, context) || node.isNodeType("jnt:file")) {
            logger.info("Skip {} [{}]: not displayable or is file", node.getPath(), language); return;
        }

        String url = computeVanityUrl(node, language, site.getDefaultLanguage());
        if (url == null) {
            logger.info("Skip {} [{}]: null URL computed", node.getPath(), language); return;
        }
        ...
        if (!forceRegen && mode == PermalinkMode.SMART && hasManualActiveDefaultVanity(node, language)) {
            logger.debug("Skip {} [{}]: manual vanity exists", node.getPath(), language); return;
        }

        String candidateUrl = resolveUniqueUrl(url, node, site.getSiteKey(), session);
        if (candidateUrl == null) return;

        // Already correct — avoid remove+re-add which fails when vanity is live-published
        if (!forceRegen && hasAutoGeneratedActiveDefaultVanity(node, language, candidateUrl)) return;
     ...
     */
    public int generateVanityForNodeIds(List<String> nodeIds, List<String> languages, JCRSessionWrapper session) {
        return generateVanityForNodeIds(nodeIds, languages, session, false);
    }

    public int generateVanityForNodeIds(List<String> nodeIds, List<String> languages, JCRSessionWrapper session, boolean forceRegen) {
        int count = 0;
        for (String nodeId : nodeIds) {
            for (String language : languages) {
                try {
                    ...
                    updateVanityForNode(node, language, site, langSession, forceRegen);
                    count++;
                } catch (Exception e) {
