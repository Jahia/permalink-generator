...
public class PermalinkGeneratorService {
    ...
    private void updateVanityForNode(JCRNodeWrapper node, String language, JCRSiteNode site, JCRSessionWrapper session) throws RepositoryException {
        ...
        // SMART: respect existing manual default vanity — don't override SEO URLs set by editors
        if (mode == PermalinkMode.SMART && hasManualActiveDefaultVanity(node, language)) {
        ...
        for (String nodeId : nodeIds) {
            for (String language : languages) {
                try {
                    ...
                    logger.info("generateVanityForNodeIds: processing {} [{}] path={}", nodeId, language, node.getPath());
                    updateVanityForNode(node, language, site, langSession);
                    count++;
                } catch (Exception e) {
